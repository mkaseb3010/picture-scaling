...
public class PaintPanel extends View
{
     ...
     * keeping track of multiple contact points in the onTouchEvent method.
     */
    ...
    private boolean isDoubleTapped; //added a boolean statement to check if the image has been double tapped or not
    ...
    private void initialize(Context context)
    {
        ...
        targetImage = context.getResources().getDrawable(R.drawable.Insert_Your_Image_Here);
    ...
    public boolean onTouchEvent(MotionEvent me)
    {
        ...
        switch (action & MotionEvent.ACTION_MASK)
        {
            ...
            case MotionEvent.ACTION_DOWN:
            {
                ...
                // this determines if the initial touch point is inside the image
    ...
    private class MyGestureListener extends GestureDetector.SimpleOnGestureListener
    {
        ...
        /*
         * onDoubleTap - This method is designed to get the coordinates using float to accurately zoom in on
         * the part of the image in which it is being clicked, resets the image back to how it was before it
         * was zoomed in. The goal is to determine whether the image is being double tapped on or not, and to
         * zoom in to exactly where in the image in being clicked.
         */
        @Override
        public boolean onDoubleTap(MotionEvent e) {
            final float x = e.getX();
            final float y = e.getY();
            final float top = yPosition;
            final float bottom = top + imageIntrinsicHeight * scaleFactor;
            final float left = xPosition;
            final float right = left + imageIntrinsicWidth * scaleFactor;

            final float scale = 3.0f;
            final float xUpdate = 3f;
            final float yUpdate = 3f;

            RectF image = new RectF(left, top, right, bottom);

            if (image.contains(x, y)) {
                if (isDoubleTapped) {
                    scaleFactor = scaleFactor / scale;
                    xPosition = (xPosition - x)/xUpdate + x;
                    yPosition = (yPosition - y)/yUpdate + y;
                } else {
                    scaleFactor = scaleFactor * scale;
                    xPosition = (xPosition - x)*xUpdate + x;
                    yPosition = (yPosition - y)*yUpdate + y;
